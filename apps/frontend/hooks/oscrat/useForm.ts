...
export function useForm({ teamSlug, productId }: UseFormOptions) {
  ...
  const [initialFormState, setInitialFormState] =
    useState<Partial<FormState> | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);

  // Fetch product
  const { project, updateProject } = useOscratProject(
    teamSlug || '',
    productId || '',
  ...
  const latestCRAAssessment = useMemo(() => {
    ...
    const craAssessments = assessmentsResponse.filter(
  ...
  const { createAssessment } = useAssessments(teamSlug || '', { productId });
  ...
  const handleAssessmentUpdate = useCallback(
    async (formState: FormState) => {
      ...
      try {
        const newCategory = getProductCategoryFromRisk(
          formState.highestRiskLevel as RiskLevel
        );
        ...
        toast.error(
          extractErrorMessage(error, t('oscrat.ui.failed-to-update-assessment'))
        );
