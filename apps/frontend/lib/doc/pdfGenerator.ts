...
export function buildPDFDocTranslations(t: TFunction): PDFDocTranslations {
  return {
    ...
    declarationOfResponsibility: t('oscrat.ui.doc.pdf.declaration-of-responsibility'),
    declarationResponsibilityText: t('oscrat.ui.doc.pdf.declaration-responsibility-text'),
...
export async function generateDocPDF({
  ...
  if (declarationType === 'simple') {
    ...
    const manufacturer = simpleData.manufacturerName || translations.notProvided;
    ...
    const addSection = (title: string, content: string) => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      const lines = doc.splitTextToSize(content || translations.notProvided, contentWidth);
      doc.text(lines, margin, y);
      y += lines.length * 5 + 8;
    };
    ...
    addSection(`3. ${translations.declarationOfResponsibility}`, translations.declarationResponsibilityText);
    addSection(`4. ${translations.objectOfDeclaration}`, fullData.productDescription);
    addSection(`5. ${translations.statementOfConformity}`, fullData.harmonisationLegislation);
    addSection(`6. ${translations.standardsSpecifications}`, fullData.standardsAndSpecs);
    addSection(`7. ${translations.notifiedBodyInvolvement}`, fullData.notifiedBodyInfo);
    addSection(`8. ${translations.additionalInformation}`, fullData.additionalInfo);
    ...
    y += 8;
    doc.setFont('helvetica', 'normal');
    doc.text(`${translations.signedForOnBehalf}: ${fullData.manufacturerName}`, margin, y);
    y += 6;
    doc.text(`${translations.placeAndDate}: ${fullData.placeAndDate}`, margin, y);
    y += 6;
    doc.text(`${translations.name}: ${fullData.signatoryName}`, margin, y);
    y += 6;
    doc.text(`Function: ${fullData.signatoryFunction}`, margin, y);
    y += 6;
    doc.text(`${translations.signatureLine}: ________________________`, margin, y);
  }
