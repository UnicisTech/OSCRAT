...
export default function Index() {
  ...
  const [vulnerabilityToDelete, setVulnerabilityToDelete] = useState<
    string | null
  >(null);
  const [statusFilter, setStatusFilter] = useState<string>('All');

  const { vulnerabilities, isLoading, isError, error, deleteVulnerability } =
    useVulnerabilities(teamId, productId, versionId);

  const filteredVulnerabilities = useMemo(() => {
    if (!vulnerabilities || statusFilter === 'All')
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        t('oscrat.ui.versions.vulnerabilities.deleted-successfully')
      );
      ...
      toast.error(
        extractErrorMessage(
          error,
          t('oscrat.ui.versions.vulnerabilities.failed-to-delete')
        )
      );
    }
  };
  ...
  if (isError) {
    return (
      <div className="text-danger">
        {extractErrorMessage(
          error,
          t('oscrat.ui.versions.vulnerabilities.failed-to-load-vulnerabilities')
        )}
      </div>
    );
  }
