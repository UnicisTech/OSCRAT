...
export default function Index() {
  ...
  const { incidents, isLoading, isListError, listError, deleteIncident } =
    useIncidents(teamId, productId, versionId);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error(
        extractErrorMessage(
          error,
          t('oscrat.ui.versions.incidents.failed-to-delete')
        )
      );
    }
  };
  ...
  if (isListError) {
    return (
      <div className="text-danger">
        {extractErrorMessage(
          listError,
          t('oscrat.ui.versions.incidents.failed-to-load-incidents')
        )}
      </div>
    );
  }
