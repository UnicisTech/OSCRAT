...
const handleGET = async (
  ...
  const report = await getSbomReportDetailsById(
    prisma,
    teamMember.teamId,
    reportId as string
  );
  ...
  if (!report) {
    throw new ApiError(404, 'SBOM report not found');
  }

  console.log(
    `[SBOM Reports API] Retrieved report: reportId: ${reportId}, teamId: ${teamMember.teamId}`
  );

  res.status(200).json({ data: report });
};

// DELETE: Delete an SBOM report
